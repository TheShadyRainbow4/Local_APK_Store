...
# Dynamically pull all loaded assemblies to ensure PowerShell 7 / .NET Core 'Primitives' are passed to the C# compiler
$eliteAssemblies = [AppDomain]::CurrentDomain.GetAssemblies() | Where-Object { -not $_.IsDynamic -and $_.Location } | Select-Object -ExpandProperty Location
...
public class Win32 {
    ...
    public static void EnableLegacyDragDrop(IntPtr handle) {
        // Explicitly punch a hole in UIPI for legacy Win32 drop messages
...
$supportedExts = @('.dll', '.mui', '.mun', '.cpl', '.exe', '.ocx')
...
function Sign-File {
    ...
    try {
        ...
        Write-Log "Failed to secure a temporary backup for $($fileName). Aborting to save your bits." "error"
        return $false
    }
    ...
    if ($useSignTool) {
        ...
        try {
            ...
            Write-Log "Native signing blew up unexpectedly: $($_.Exception.Message)" "error"
        }
    }
...
try {
    ...
    $form.Text = "Elite EasySigner - v1.0.50.0"
    ...
    $lblHeaderSub.Text = "v1.0.50.0 - EliteSoftware Certificate Signing Utility"
